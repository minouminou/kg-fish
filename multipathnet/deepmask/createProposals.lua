...
cmd:option('-savedir', 'dir/', 'save directory')
...
local savedir = string.format('%s/eval/%s/',config.model,config.split)
print(string.format('| saving results results in %s',savedir))
if config.save then os.execute(string.format('mkdir -p %s',savedir)) end
if config.save then os.execute(string.format('mkdir -p %s/t7',savedir)) end
if config.save then os.execute(string.format('mkdir -p %s/jsons',savedir)) end
...
for k = config.startAt,config.endAt do
  ...
  -- get top proposals
  local masks,scr = infer:getTopProps(config.thr,h,w)
  ...
  table.insert(images, img.file_name)
...
--props.scores = scores
props.boxes = boxes
torch.save(string.format('%s/%s.t7',config.savedir,config.split),props)
